using System; using System.Collections.Generic; using System.Globalization; using System.Linq; using System.Text; using System.Threading; using System.Threading.Tasks; using Android; using Android.App; using Android.Content; using Android.Gms.Maps; using Android.Gms.Maps.Model; using Android.Graphics;
using Android.Locations;
using Android.OS; using Android.Runtime; using Android.Support.V4.App; using Android.Support.V7.App; using Android.Views; using Android.Widget; using CardsAndroid.NativeClasses; using CardsPCL; using CardsPCL.CommonMethods; using CardsPCL.Database; using CardsPCL.Localization; using Plugin.Geolocator; using Plugin.Geolocator.Abstractions; using Plugin.Permissions; using Plugin.Permissions.Abstractions; using static Android.Gms.Maps.GoogleMap;  namespace CardsAndroid.Activities {     [Activity(ScreenOrientation = Android.Content.PM.ScreenOrientation.Portrait)]     public class NewCardAddressMapActivity : Activity, IOnMapReadyCallback, IOnMapLongClickListener     {         Methods _methods = new Methods();         DatabaseMethods _databaseMethods = new DatabaseMethods();         //NativeMethods nativeMethods = new NativeMethods();         private GoogleMap _map;         private MapFragment _mapFragment;         RelativeLayout _hintRl, _backRl;         Button _okBn, _neverShowBn, _centerPositionBn, _applyAddressBn;         TextView _hintTv, _headerTv;         float _zoom = 16;         LatLng _moscowPosition, _point;         MarkerOptions _moscowMarker, _myMarker;         Marker _workMarker;         BitmapDescriptor _userMarker, _workDescriptor;
        Position _myPosition = new Position();         MarkerOptions _markerOptions = new MarkerOptions();         private CameraUpdate _cameraUpdate;         public static string Lat = null;         public static string Lng = null;
        string _latTemporary, _lngTemporary;         private bool _launched, _userDeclinedPermission;         CultureInfo _ci = GetCurrentCulture.GetCurrentCultureInfo();          protected override void OnCreate(Bundle savedInstanceState)
        {
            base.OnCreate(savedInstanceState);             try
            {
                SetContentView(Resource.Layout.NewCardAddressMap);
            }
            catch              {
                base.OnBackPressed();                 return;
            }
            InitElements();
        }          protected override void OnResume()         {             base.OnResume();              if (!_methods.IsConnected())             {                 NoConnectionActivity.ActivityName = this;                 StartActivity(typeof(NoConnectionActivity));                 Finish();                 return;             }              _mapFragment =                     FragmentManager.FindFragmentByTag("map") as MapFragment;             if (_mapFragment == null)             {                 GoogleMapOptions mapOptions = new GoogleMapOptions()                     .InvokeMapType(GoogleMap.MapTypeNormal)                     .InvokeZoomControlsEnabled(false)                     .InvokeCompassEnabled(true);                  Android.App.FragmentTransaction fragTx = FragmentManager.BeginTransaction();                 _mapFragment = MapFragment.NewInstance(mapOptions);                 fragTx.Add(Resource.Id.map, _mapFragment, "map");                 fragTx.Commit();             }             _mapFragment.GetMapAsync(this);              InitElements();             if (!_databaseMethods.GetShowMapHint())                 _hintRl.Visibility = ViewStates.Gone;             _okBn.Click += (s, e) =>             {                 _hintRl.Visibility = ViewStates.Gone;             };             _neverShowBn.Click += (s, e) =>             {                 _hintRl.Visibility = ViewStates.Gone;                 _databaseMethods.InsertMapHint(false);             };              _backRl.Click += (s, e) =>             {                 OnBackPressed();             };              _centerPositionBn.Click += CenterPositionBn_Click;              _applyAddressBn.Click += (s, e) =>
            {
                ...
                {
                    Lat = _myPosition?.Latitude.ToString();                     Lng = _myPosition?.Longitude.ToString();
                }
        ...
        {
            try             {                 if (String.IsNullOrEmpty(Lat) && String.IsNullOrEmpty(Lng))                 {                     var sd = await GeocodeToConsoleAsync(HomeAddressActivity.FullAddressTemp);                 }                 else                 {                     SetWorkMarker(null);                 }             }             catch { }
        }
         async Task<string> GeocodeToConsoleAsync(string address)         {             var geo = new Geocoder(this);

            var addresses = geo.GetFromLocationName(address, 1);             RunOnUiThread(() =>             {
                addresses.ToList().ForEach((addr) =>                 {                     SetWorkMarker(addr);                 });
            });
            return "";         }          void SetWorkMarker(Android.Locations.Address addr)
        {
            LatLng searchLocation;             if(addr!=null)                 searchLocation = new LatLng(                                             Convert.ToDouble(addr.Latitude, (CultureInfo.InvariantCulture)),                                             Convert.ToDouble(addr.Longitude, (CultureInfo.InvariantCulture)));             else                 searchLocation = new LatLng(                                             Convert.ToDouble(Lat, CultureInfo.InvariantCulture),                                             Convert.ToDouble(Lng, CultureInfo.InvariantCulture));
        ...
        {             var granted = await CheckLocPermission();             if (!granted)
            {
                //var res = await checkLocPermission();                 return;
            }
            CheckLocationOn();
        }
        ...
        private bool CheckLocationOn()
        {
            if (IsGeolocationEnabled())             {
                ...
                TurnGpSon();                 return false;             }
        }

        // My method to turn the GPS on.
        public void TurnGpSon()         {
            Android.App.AlertDialog.Builder builder = new Android.App.AlertDialog.Builder(this);//, Resource.Style.AlertStyle);             builder.SetTitle(TranslationHelper.GetString("locationServicesDisabled", _ci));             builder.SetMessage(TranslationHelper.GetString("turnThemOnQuestion", _ci));             builder.SetCancelable(false);             builder.SetPositiveButton(TranslationHelper.GetString("no", _ci), (object sender1, DialogClickEventArgs e1) =>             { });             builder.SetNegativeButton(TranslationHelper.GetString("yes", _ci), (object sender1, DialogClickEventArgs e1) =>             {                 StartActivity(new Intent(Android.Provider.Settings.ActionLocationSourceSettings));             });             Android.App.AlertDialog dialog = builder.Create();             dialog.Show();         } 
        public async Task<string> Geolocation()         {             try             {                 var locator = CrossGeolocator.Current;                 locator.DesiredAccuracy = 50;                  _myPosition = await locator.GetLastKnownLocationAsync();                 if (_myPosition == null && locator.IsGeolocationAvailable && locator.IsGeolocationEnabled)                     _myPosition = await locator.GetPositionAsync(TimeSpan.FromSeconds((double)30000));             }             catch             {                 var locator = CrossGeolocator.Current;                 locator.DesiredAccuracy = 50;                 _myPosition = await locator.GetPositionAsync(TimeSpan.FromSeconds((double)30000));             }             if (_myPosition != null)             {                 SetMyMarker();             }             if (_myPosition == null)                 return "";              return "";         }
        ...
        {             RunOnUiThread(() =>
            {
                LatLng myPos = new LatLng(
                                   ...
                                   Convert.ToDouble(_myPosition.Longitude/*, (CultureInfo.InvariantCulture)*/));                 _myMarker = new MarkerOptions();
                ...
                _map.AddMarker(_myMarker);                 if(//!launched                  String.IsNullOrWhiteSpace(HomeAddressActivity.FullAddressTemp) && String.IsNullOrEmpty(Lat) && String.IsNullOrEmpty(Lng))                 {                     CenterPositionBn_Click(null, null);                     _launched = true;                 }
            });
        }          // Method to detect if GPS is enabled         public bool IsGeolocationEnabled()         {             return CrossGeolocator.Current.IsGeolocationEnabled;         }         async Task<bool> CheckLocPermission()         {             PermissionStatus locationStatus = await CrossPermissions.Current.CheckPermissionStatusAsync(Permission.Location);              if (locationStatus != PermissionStatus.Granted)             {                 var results = await CrossPermissions.Current.RequestPermissionsAsync(new[] { Permission.Location });                 locationStatus = results[Permission.Location];                 if(!_userDeclinedPermission)                     request_runtime_permissions();                 return false;             }              return true;         }          private const int requestPermissionCode = 1000;         void request_runtime_permissions()         {             if (Build.VERSION.SdkInt >= Build.VERSION_CODES.M)                 if (                                CheckSelfPermission(Manifest.Permission.AccessFineLocation) != Android.Content.PM.Permission.Granted                           || CheckSelfPermission(Manifest.Permission.AccessCoarseLocation) != Android.Content.PM.Permission.Granted                           || CheckSelfPermission(Manifest.Permission.AccessMockLocation) != Android.Content.PM.Permission.Granted)                 {                     ActivityCompat.RequestPermissions(this, new String[]                     {                                 Manifest.Permission.AccessFineLocation,                                 Manifest.Permission.AccessCoarseLocation,                                 Manifest.Permission.AccessMockLocation,                     }, requestPermissionCode);                 }                 else                 {                     ActivityCompat.RequestPermissions(this, new String[]                     {                                 Manifest.Permission.AccessFineLocation,                                 Manifest.Permission.AccessCoarseLocation,                                 Manifest.Permission.AccessMockLocation,                     }, requestPermissionCode);                 }         }         bool _shown = false;


        public override void OnRequestPermissionsResult(int requestCode, string[] permissions, [GeneratedEnum] Android.Content.PM.Permission[] grantResults)         {             //base.OnRequestPermissionsResult(requestCode, permissions, grantResults);             switch (requestCode)             {                 case requestPermissionCode:                     {                         if (grantResults.Length > 0 && grantResults[0] == Android.Content.PM.Permission.Granted)                         {                             CheckLocationOn();                         }                         else                         {                             _userDeclinedPermission = true;                             if (!_shown)                                 Toast.MakeText(this, TranslationHelper.GetString("permissionsNeeded", _ci), ToastLength.Long).Show();                             _shown = true;                             //request_runtime_permissions();                         }                         break;                     }             }         }         void IOnMapLongClickListener.OnMapLongClick(LatLng point)         {             //this.point = point;             _workDescriptor = BitmapDescriptorFactory.FromResource(Resource.Drawable.add_loc_marker);             if (_workMarker != null)             {                 _workMarker.Remove();             }             //edit_city_coord_for_edit = city_coord_for_edit_prefs.Edit();             //apply_cancelLL.Visibility = ViewStates.Visible;             var markerOptions = new MarkerOptions();             markerOptions.SetPosition(point);             markerOptions.SetIcon(_workDescriptor);                          _workMarker = _map.AddMarker(markerOptions);             _latTemporary = point.Latitude.ToString().Replace(',', '.');             _lngTemporary = point.Longitude.ToString().Replace(',', '.');
